...
export HOSTS_IPS="[$ascs_private_ip,$ers_private_ip]"
...
ascs_overlay_route_table_id=$(terraform -chdir="$PWD/$TERRAFORM_FOLDER_NAME" output -json ascs_overlay_ip_route_table_id)
if [ -z "$ascs_overlay_route_table_id" ]; then
    echo "No ID for the overlay IP route table was found for Hana. Please check Terraform step"
    exit 105
fi

ers_overlay_route_table_id=$(terraform -chdir="$PWD/$TERRAFORM_FOLDER_NAME" output -json ers_overlay_ip_route_table_id)
if [ -z "$ers_overlay_route_table_id" ]; then
    echo "No ID for the overlay IP route table was found for Hana. Please check Terraform step"
    exit 105
fi
